...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
        ...
        if (ISIE7 || ISIE8)
            $filename = 'filename=' . urlencode(_basename($ame));
        if (ISSafari)
            $filename = 'filename="' . _basename($name) . '"';
        else 
            $filename = 'filename="' . _basename($name) . '"; filename*=UTF-8\'\'' . urlencode(_basename($name));
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	...
	public function getFontFamilyName($fontfamily) {
		...
		$fontfamily = preg_replace('/[^a-z0-9_\,]/', '', strtolower('ms_gothic'));
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// stroke
		if ($svgstyle['stroke'] != 'none' && $svgstyle['stroke-opacity'] > 0) {
