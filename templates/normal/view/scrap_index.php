...
<html lang="ja">
  ...
  <body>
	<div class="container">
		<form id="form" action="scrap/scrap" class="form-signin form-horizontal" method="post">
			<div class="masthead">
				<h3 class="logo"><?php p(PRODUCT_NAME); ?></h3>
				<h1><?php l("全柔連国内ポイントシステムスクラップページ");?>
					<p><?php l("このページは全柔連国内ポイントシステムのスクラップを案内します。");?></p></h1>
			</div>
			<div class="main">
				<div class="row">
					<div class="span12">
			...
			$('#form').ajaxForm({
				...
				success: function(ret, statusText, xhr, form) {
					try {
						if (ret.err_code == 0)
						{
							alertBox("<?php l('スクラップ完了');?>", "<?php l('システムスクラップが完了しました。');?>", function() {
								goto_url("sysman/setting");
							});
						}
						else {
							errorBox("<?php l('スクラップ失敗');?>", "<?php l('すみません。スクラップが失敗しました。');?>");
						}
					}
					finally {
					}
